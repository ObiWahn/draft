...
is a type \tcode{T3}
similar to \tcode{T1} whose \deflink{cv-qualification signature}{conv.qual} is:
\begin{itemize}
\item
...
reference-related to \tcode{C2} or \tcode{C2} is \deflink{reference-related}{dcl.init.ref} to
...
argument of \link{class type}{class} having a non-trivial
...
\link{derived}{class.derived} from \tcode{B},
...
The \link{lvalue-to-rvalue}{conv.lval},
\link{array-to-pointer}{conv.array}, and
\link{function-to-pointer}{conv.func} standard conversions are not
...
which is an \deflink{unevaluated operand}{expr.prop}, can \link{throw an
...
\deflinkx{potentially-throwing}{potentially-throwing!expression}{except.spec}.
\indextext{expression!unary|)}
...
\link{pointer to member conversions}{conv.mem} and \link{qualification
conversions}{conv.qual} are performed to bring them to their
\deflink{composite pointer type}{expr.type}. The result is of the composite pointer type.
...
machine}{intro.execution}, would evaluate one of the following expressions:

\begin{itemize}
\item
\link{\tcode{this}}{expr.prim.this}, except in a constexpr
...
\begin{note} \link{Overload resolution}{over.match}
...
\link{constexpr}{dcl.constexpr} constructor;

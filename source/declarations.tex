...
contain only the \grammarterm{type-specifier} \link{\tcode{auto}}{dcl.spec.auto}
...
shall not be
specified in an \link{explicit specialization}{temp.expl.spec} or an
\link{explicit instantiation}{temp.explicit} directive.
...
function or to the declaration of an \link{anonymous union}{class.union.anon}. There can be no
...
appertains to a \link{friend declaration}{class.friend}, that declaration shall be a
definition. No \grammarterm{attribute-specifier-seq} shall appertain to an
\link{explicit instantiation}{temp.explicit}.
...
respectively).
An \grammarterm{alignment-specifier} with an ellipsis is a \deflink{pack expansion}{temp.variadic}.
...
\pnum
During \link{constant expression}{expr.const} evaluation,
...
how
the \link{\tcode{std::contract_violation}}{support.contract.cviol} argument value
is set,
...
\begin{note}
The function \link{\tcode{std::terminate}}{except.terminate} is invoked.
\end{note}
\begin{example}
\begin{codeblock}
...
a potentially-evaluated \deflink{discarded-value expression}{expr.prop}

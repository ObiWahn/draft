...
shall be an \link{explicit specialization}{temp.expl.spec} or
a \link{partial specialization}{temp.class.spec}.
...
\begin{itemize}
\item where each
\link{copy constructor}{class.copy.ctor},
\link{move constructor}{class.move.ctor},
\link{copy assignment}{class.copy.assign} operator, and
\link{move assignment}{class.move.assign} operator
...
\item that has a trivial, non-deleted \link{destructor}{class.dtor}.
\end{itemize}

\pnum
A \defnadj{trivial}{class} is a class that is trivially copyable and
has one or more \link{default constructors}{class.default.ctor},
...
\begin{itemize}
\item \link{function body}{dcl.fct.def.general},
\item \link{default argument}{dcl.fct.default},
\item \grammarterm{noexcept-specifier}\iref{except.spec},
\item \deflink{contract condition}{dcl.attr.contract}, or
...
an \link{abstract class type}{class.abstract},
...
with the same \link{access control}{class.access}
...
with different \link{access control}{class.access}
is unspecified.
...
for space for managing \link{virtual functions}{class.virtual} and
virtual \link{base classes}{class.mi}.
...
Member functions of a \link{local class}{class.local} have no linkage.
See~\ref{basic.link}.
\end{note}
...
declared elsewhere.\footnote{See, for example, \link{\tcode{<cstring>}}{c.strings}.}
\end{example}
...
\link{virtual}{class.virtual} or \link{pure virtual}{class.abstract}.
...
The \link{default constructor}{class.default.ctor},
\link{copy constructor}{class.copy.ctor}, move constructor,
\link{copy assignment}{class.copy.assign} operator, move assignment operator,
and \link{destructor}{class.dtor} are
...
if they are \link{odr-used}{basic.def.odr} or
needed for \link{constant evaluation}{expr.const}.
\end{note}
...
Special member functions obey the usual \link{access rules}{class.access}.
\begin{example}
...
and, if the class is not \link{abstract}{class.abstract}, its virtual base
...
\link{function declarator}{dcl.fct} of the form
\begin{ncbnf}
...
but is not a \link{friend}{class.friend}
declaration, the \grammarterm{id-expression} is the
...
name lookup; however an explicit \link{type conversion}{expr.type.conv} using the functional
...
and
\link{\tcode{volatile}
semantics}{dcl.type.cv} are not applied on an object under construction.
They come into effect when the constructor for the
most derived \link{object}{intro.object} ends.
...
\item any non-static data member with no default \link{member}{class.mem} initializer is
of reference type,
...
has no default constructor or \link{overload resolution}{over.match}
...
a default \link{member}{class.mem} initializer, and
...
when it is \link{odr-used}{basic.def.odr}
to create an object of its class type\iref{intro.object},
when it is needed for \link{constant evaluation}{expr.const}, or
...
\link{\grammarterm{ctor-initializer}}{class.base.init} and an empty
...
of a \link{constexpr constructor}{dcl.constexpr}, the implicitly-defined
...
without an \link{initializer}{dcl.init},
are called to create class objects of \link{dynamic storage duration}{basic.stc.dynamic} created by a
\grammarterm{new-expression}
in which the
\link{\grammarterm{new-initializer}}{expr.new}
is omitted, or
are called when the explicit \link{type conversion}{expr.type.conv}syntax is
used.
A program is ill-formed if the default constructor for an object
is implicitly used and the constructor is not \link{accessible}{class.access}.
...
or else all other parameters have \link{default arguments}{dcl.fct.default}.
\begin{example}
\tcode{X::X(const X\&)}
and
\tcode{X::X(X\&,int=1)}
